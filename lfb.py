...
class Draw:
    ...
    def lsdir(self):
        ...
        if not self._show_hidden_files:
            files = [file for file in files if not file.startswith(".") or file == ".gitignore"] 
    ...
    def print_file(self, file: str, index: int):
        ...
        if self._display_index == index:
            print(f"{leading_space}\033[K\033[{icon_colour}m{icon} \033[0m\033[30;47m{file}\033[0m{'/' if is_dir else ''}")
        else:
            print(f"{leading_space}\033[K\033[{icon_colour}m{icon} \033[0m\033[{file_colour}m{file}\033[0m{'/' if is_dir else ''}") 
    ...
    def draw_footer(self):
        ...
        if len(files) != 0:
            ...
            self.colour(f"\033[K{self._selected_index+1}/{len(self.lsdir())} {time.strftime(config.DATE_FORMAT, time.gmtime(os.stat(file).st_mtime))} {formatted_size}", config.FOOTER_COLOUR)
        else:
            self.colour("0/0", "34")
...
class Lfb:
    ...
    def back_directory(self):
        ...
        if self.cwd_files_length() > os.get_terminal_size()[1]-6:
            self._draw._display_index = 2 if self._draw._selected_index-2 > 0 else self._draw._selected_index
            self._draw._min_view = self._draw._selected_index-2 if self._draw._selected_index-2 > 0 else 0
        else:
    ...
    def delete(self):
        ...
        if len(self._draw._selected[cwd]) > 0:
            for file in self._draw._selected[cwd]:
                ...
                file_index = [i for i, f in enumerate(self._draw._selected[cwd]) if f == file] 
                if len(file_index) > 0:
                    file_index = file_index[0]
                    c = self._draw._selected[cwd].pop(file_index)
    ...
    def scroll_up(self):
        files_length = self.cwd_files_length()
        if self._draw._selected_index != 0:
            self._draw._selected_index -= 1 
            self._draw._display_index -= 1
    ...
    def keys(self):
        ...
        if c == 65 or c == 107:
            ...
            if self._draw._min_view != 0:
                ...
                self._draw._display_index = 0
            self._draw._selected_index = self._draw._min_view+self._draw._display_index
            self._draw.draw_files()
            self._draw.draw_footer()
        elif c == 76:
            ...
            if self.cwd_files_length()-1 < bottom_size:
                ...
                self._draw._min_view = self.cwd_files_length()-(os.get_terminal_size()[1]-6)
            ...
            if cmd != "":
                ...
                self._draw._selected_index = [i for i, f in enumerate(files) if f.startswith(cmd)][0]
            ...
            self._draw._show_hidden_files = False if self._draw._show_hidden_files else True
            self._draw._selected_index = 0
            self._draw._display_index = 0
            self.draw_screen()
